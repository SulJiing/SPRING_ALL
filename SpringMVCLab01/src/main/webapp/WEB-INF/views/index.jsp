...
<pre>
	1. @Controller 어노테이션으로 컨트롤러 객체를 bean으로 등록
	2. 핸들러 메소드 등록 및 구현
	...
	3. 핸들러 메소드 내에서 모델과 뷰에 대한 정보가 결정됨
		...
		- 생성된 모델을 뷰로 전달하는 방법
		 Model, NodelAndView등의 객체를 이용하여 전달 -> 
				핸들러 메소드가 종료될 때 해당 객체 내의 모든 attribute는 scope에 복사됨
		
		- 뷰의 결정 방법
		 logical view name을 핸들러 메소드의 반환 값으로 설정
		 response content type에 따른 뷰 결정 -> request accept 헤더에 따라 스프링 내부에서 뷰가 자동으로 결정됨
</pre>
</body>
</html>