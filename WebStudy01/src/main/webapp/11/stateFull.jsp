...
<body>
<h4>HTTP (ConnectLess, StateLess)보완하고, 상태를 유지하기 (StateFull)위한 저장구조</h4>
<pre>
	1. session : 상태유지를 위한 정보가 서버측에 저장되는 개념.
		...
		세션의 생명주기
			생성		: 한 클라이언트가 사용하고 있는 하나의 에이전트(브라우저)에 대해 최초의 요청이 발생했을 때 세션이 생섬됨.
					  각 세션은 식별자로 세션 아이디를 발급받음.
					  세션 아이디 	: <%=session.getId() %>
					  세션 생성시점 : <%=new Date(session.getCreationTime()) %>
			유지 방법	: 서버와 클라이언트가 동일한 세션 아이디를 공유하는 상황.(=tracking mode)
				세션 timeout1 : <%=session.getMaxInactiveInterval() %>s
					<%
						session.setMaxInactiveInterval(4*60);
					%>
				세션 timeout2 : <%=session.getMaxInactiveInterval() %>s
				마지막 요청 시점 : <%=new Date(session.getLastAccessedTime()) %>
				1) COOKIE : request/respnse 헤더를 통해 세션 아이디 공유
				2) URL	  : request line을 통해 세션 아이디 공유(보안 취약)
					<a href="stateFull.jsp;jsessionid=<%=session.getId()%>">세션 파라미터를 통한 세션유지</a>
				3) SSL(Secure Socket Layer): (https)암호화 전송구조를 이용한 공유 
			소멸		: 만료 시간(timeout)이내에 새로운 요청이 발생하지 않는 경우
				1) 브라우저 종료
				2) 쿠키 삭제
				3) 직접 로그아웃(session invalidate)
				<%
// 					session.invalidate(); // 세션이 바로 지워짐
				%>
	
	2. cookie  : 상태유지를 위한 정보가 클라이언트측에 저장되는 개념.
	- server
		1) 쿠키 객체 생성 : name/value 속성(기본생성자가 없음), value에 한글을 비롯한 특수문자가 포함된 경우 -> URLEncoding 과정이 필요함
		2) 쿠키의 부가 속성 설정
			- name/value (required)
			- domain  : 생략한 경우, 쿠키를 생성할 때 도메인이 반영됨
				   	    쿠키의 재전송 여부를 결정하는 조건
			...
			- maxAge  : 쿠키의 저장 시한 결정(초단위)
						ex) 60 * 60 *24 * 7 : 일주일
							0 : 저장하지 말고 저장되어 있는 쿠키도 모두 삭제 = 쿠키 삭제
							-1 : 브라우저 종료시 쿠키 삭제
			- httpOnly: 클라이언트 사이드 모듈에서 접근을 허용하지 않을 때
